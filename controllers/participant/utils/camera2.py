...
class Camera2():
    ...
    def __init__(self, robot, camera_name = 'CameraBottom'):
        """Initialize the image processing class."""
        self.robot = robot
        self.camera2 = robot.getDevice(camera_name)
        self.camera2.enable(robot.time_step)
        self.height = self.camera2.getHeight()
        self.width = self.camera2.getWidth()

    def get_image(self):
        """Get an openCV image (BGRA) from a Webots camera."""
        return np.frombuffer(self.camera2.getImage(), np.uint8).reshape((self.height, self.width, 4))
